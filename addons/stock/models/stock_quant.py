    ...
    def _update_reserved_quantity(self, product_id, location_id, quantity, lot_id=None, package_id=None, owner_id=None, strict=False):
        ...
        if float_compare(quantity, 0, precision_rounding=rounding) > 0:
            ...
            if float_compare(abs(quantity), available_quantity, precision_rounding=rounding) > 0:
                self._fix_quants_for_specific_product(product_id)
                raise UserError(_('It is not possible to unreserve more products of %s than you have in stock.'
                                  'System has tried to fix quant issue, run action again, if problem persist contact an administrator.'
...
class QuantPackage(models.Model):
    ...
    @api.model
    def _fix_quants_for_specific_product(self, product_id):
        ...
        if len(move_lines_to_unreserve) > 1:
            self.env.cr.execute(""" UPDATE stock_move_line SET product_uom_qty = 0, product_qty = 0 WHERE id in %s ;""" % (
            tuple(move_lines_to_unreserve),))
        elif len(move_lines_to_unreserve) == 1:
            self.env.cr.execute(""" UPDATE stock_move_line SET product_uom_qty = 0, product_qty = 0 WHERE id = %s ;""" % (
